...
async function main() {
    ...
    await prisma.coupon.createMany({
        data: [
            ...
            {
                code: 'SAVE10',
                type: client_1.CouponType.PERCENT,
                value: 10,
                minOrderValue: 500,
                maxUses: 50,
                isActive: true,
    ...
    const napaProduct = products[0];
    const vitaminC = products[4];
    ...
    const order2 = await prisma.order.create({
        data: {
            ...
            addressSnapshot,
            paymentMethod: client_1.PaymentMethod.SSLCOMMERZ,
            items: {
                create: [
                    {
            ...
            statusLogs: {
                create: [
                    { status: client_1.OrderStatus.PENDING, note: 'Order placed' },
                    { status: client_1.OrderStatus.CONFIRMED, note: 'Payment received via SSLCommerz. Val ID: SSL-VAL-001' },
                ],
            },
            payment: {
                create: {
    ...
    const order3 = await prisma.order.create({
        data: {
            ...
            statusLogs: {
                create: [
                    { status: client_1.OrderStatus.PENDING, note: 'Order placed' },
                    { status: client_1.OrderStatus.CONFIRMED, note: 'Confirmed by pharmacist' },
                    { status: client_1.OrderStatus.PROCESSING, note: 'Being packed' },
                    { status: client_1.OrderStatus.SHIPPED, note: 'Handed to delivery agent' },
                    { status: client_1.OrderStatus.DELIVERED, note: 'Delivered successfully' },
                ],
            },
            payment: {
                create: {
    ...
    await prisma.order.create({
        data: {
            ...
            statusLogs: {
                create: { status: client_1.OrderStatus.PENDING, note: 'Order placed — awaiting payment' },
            },
            payment: {
                create: {
                    method: client_1.PaymentMethod.SSLCOMMERZ,
